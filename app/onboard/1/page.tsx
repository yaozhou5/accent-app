        ...
        <div className="max-w-[480px] w-full py-16">
          {loading || !result ? (
            <div className="text-center">
              <div className="inline-block w-6 h-6 rounded-full border-2 mb-4 animate-spin" style={{ borderColor: `${BORDER} ${BLUE} ${BLUE}` }} />
              <p className="font-sans" style={{ fontSize: 16, color: DIM }}>Reading what you told me...</p>
            </div>
          ) : (
            <>
    ...
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: "#fff" }}>
      <div className="max-w-[480px] w-full py-16">
        ...
        <div className="flex gap-2 mb-10">
          {[1, 2, 3, 4].map(s => (
            <div key={s} className="flex-1 h-[3px] rounded-full" style={{ background: s <= step ? INK : BORDER, transition: "background-color 0.2s ease" }} />
          ))}
        </div>

        <span className="font-mono text-[11px] uppercase block mb-2" style={{ color: DIM, letterSpacing: "0.1em" }}>{step} of 4</span>
        <h1 className="font-serif mb-2" style={{ fontSize: 22, fontWeight: 600, color: INK, lineHeight: 1.4 }}>{current.q}</h1>
        {current.sub && (
          <p className="font-sans mb-6" style={{ fontSize: 14, color: FAINT, lineHeight: 1.5 }}>{current.sub}</p>
        )}
        ...
        <div className="flex gap-3">
          <button
            ...
            style={{ background: BLUE, color: "#fff", border: "none", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>
            {step === 4 ? (q4.trim() ? "Finish" : "Skip & finish") : "Next"}
          </button>
          {step > 1 && (
            <button onClick={() => setStep(step - 1)}
              className="px-6 py-3 rounded-full font-sans text-[14px]"
              style={{ border: `1px solid ${BORDER}`, color: DIM, background: "#fff", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>
              Back
            </button>
          )}
        </div>
